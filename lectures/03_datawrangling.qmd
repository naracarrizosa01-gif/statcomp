...
```{r, warning=FALSE}
library(tidyverse)    # also loads package dplyr
library(gapminder)    # data for examples
library(Lahman)       # data for examples
library(nycflights13) # data for examples
```
...
class(flightsdf)
filter(flightsdf, month == 1 & day == 1)
```
...
Trick to put some columns at the start. 

:::panel-tabset

### select

Select them, then use `everything`
...
```{r}
relocate(flights, time_hour, air_time) # .before & .after arguments to place other columns before/after
```
...
```{r}
delays= filter(flights, !is.na(dep_delay), !is.na(arr_delay)) |>
  group_by(tailnum) |>
...
ggplot(delays, aes(x=count, y=delay)) +
  geom_point() +
  geom_smooth() +
  labs(x='Number of flights', y='Mean arrival delay') +
...
```{r, eval=FALSE}
library(gapminder) # load the gapminder data
gapminder
```
